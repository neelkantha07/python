...
tickers = ["RELIANCE.NS", "TCS.NS", "HdataCBANK.NS", "BHARTIARTL.NS", "ICICIBANK.NS", "SBIN.NS", "INFY.NS", "UNITDSPR.NS"]
...
def check_20_day_high():
    ...
    for ticker in tickers:
        try:
            ...
            if data['Close'].iloc[-1] > data['20_day_high'].iloc[-1]:
...
def check_20_day_low():
    ...
    for ticker in tickers:
        try:
            ...
            if data['Close'].iloc[-1] < data['20_day_low'].iloc[-1]:
                matching_tickers.append(ticker)
...
def check_higher_highs():
    ...
    for ticker in tickers:
        try:
            ...
            if (data['High'].iloc[-1] > data['previous_high'].iloc[-1]):
                matching_tickers.append(ticker)
...
def check_bollinger_band_crossings_down():
    ...
    for ticker in tickers:
        try:
            ...
            if (data['Close'].iloc[-1] < data['bb_lower_band'].iloc[-1]) :
                matching_tickers.append((ticker,calculate_percentage_change(data['Close'].iloc[-1],data['bb_lower_band'].iloc[-1])))
...
def check_bollinger_band_crossings_up():
    ...
    for ticker in tickers:
        try:
            ...
            if (data['Close'].iloc[-1] > data['bb_upper_band'].iloc[-1]) :
                matching_tickers.append((ticker,calculate_percentage_change(data['Close'].iloc[-1],data['bb_upper_band'].iloc[-1])))
...
def check_double_bottom():
    ...
    for ticker in tickers:
        try:
            ...
            data['min_20d'] = data['Low'].rolling(window=60).min()
            data['min_20d_count'] = data['Low'].rolling(window=20).apply(lambda x: list(x).count(min(x)))
            if data['min_20d_count'].iloc[-1] >= 2:
                matching_tickers.append(ticker)
...
def RS():
    ...
    # Fetch data
    base_data = yf.download(base_ticker, period=period, interval=interval, group_by='ticker', progress=True)
    
    
    if isinstance(base_data.columns, pd.MultiIndex):
        base_data = base_data.xs(base_ticker, axis=1, level=0)
     
    for ticker in tickers:
        try:
            comparative_data = yf.download(ticker, period=period, interval=interval, group_by='ticker', progress=True)
            
            if comparative_data.empty:
                continue
            if isinstance(comparative_data.columns, pd.MultiIndex):
                comparative_data = comparative_data.xs(ticker, axis=1, level=0)
...
def get_data(ticker, start_date, end_date):
    return  yf.download(ticker, start=start_date, end=end_date, group_by='ticker', progress=False)
...
# Set dates
    ...
    for ticker, data in data.items():
        try:
            ...
            # Last 1-year return
            one_year_return = (data['Close'][-1] / data['Close'][-252] - 1) * 100
    
            # 52-week high
            high_52_week = data['Close'][-252:].max()
            within_20_pct_high = data['Close'][-1] >= high_52_week * 0.8
    
            # More than 50% up days in the last 6 months (126 trading days)
            six_month_data = data['Close'][-126:]
            up_days = (six_month_data.pct_change() > 0).sum()
            up_days_pct = up_days / len(six_month_data) * 100
    
            # Filtering criteria
            if (data['Close'][-1] >= data['EMA100'][-1] >= data['EMA200'][-1] and
                one_year_return >= 6.5 and
                within_20_pct_high and
                up_days_pct > 50):
    
                # Calculate returns
                return_12m =(data['Close'][-1] / data['Close'][-126] - 1) * 100
                return_6m = (data['Close'][-1] / data['Close'][-126] - 1) * 100
                return_3m = (data['Close'][-1] / data['Close'][-63] - 1) * 100
                return_1m = (data['Close'][-1] / data['Close'][-21] - 1) * 100
    ...
    data_summary['Return_12M'] = data_summary['Return_12M'].round(1)
    data_summary['Return_6M'] = data_summary['Return_6M'].round(1)
    data_summary['Return_3M'] = data_summary['Return_3M'].round(1)
    data_summary['Return_1M'] = data_summary['Return_1M'].round(1)
    
    # Ranking based on returns
    data_summary['Rank_12M'] = data_summary['Return_12M'].rank(ascending=False)
    data_summary['Rank_6M'] = data_summary['Return_6M'].rank(ascending=False)
    data_summary['Rank_3M'] = data_summary['Return_3M'].rank(ascending=False)
    data_summary['Rank_1M'] = data_summary['Return_1M'].rank(ascending=False)
    
    # Calculate final rank
    #data_summary['Final_Rank'] = data_summary['Rank_12M'] + data_summary['Rank_6M'] + data_summary['Rank_3M'] + data_summary['Rank_1M']
    data_summary['Final_Rank'] = data_summary['Rank_12M'] + data_summary['Rank_6M'] 
    
    # Sort by final rank and get top 30
    data_summary_sorted = data_summary.sort_values('Final_Rank').head(30)
    
    # Assign position based on final rank
    data_summary_sorted['Position'] = np.arange(1, len(data_summary_sorted) + 1)
    for i, row in data_summary_sorted.iterrows():
...
# Set dates
    ...
    # Calculate final rank
    data_summary['Final_Rank'] = data_summary['Rank_6M'] + data_summary['Rank_3M'] + data_summary['Rank_1M']
    #data_summary['Final_Rank'] = data_summary['Rank_12M'] + data_summary['Rank_6M'] 
...
def display_matching_stocks():
    ...
    elif selected_action == "Momemtum631":
         matching_stocks = momemtum631()   
...
def plot_chart(event):
    ...
    data =data = yf.download(info_ticker, period=period, interval=interval, group_by='ticker', progress=True)
    
   
    if isinstance(data.columns, pd.MultiIndex):
        data = data.xs(info_ticker, axis=1, level=0)
    ...
    if chart_type == "candlestick":
        ...
        ax_price.plot(data.index, data['Close'], label=f'Close Price (Current: {current_price:.2f})', color='blue')

    # Plot EMAs
    ax_price.plot(data.index, data['EMA20'], label='20-day EMA', color='orange')
    ax_price.plot(data.index, data['EMA50'], label='50-day EMA', color='purple')
    ax_price.plot(data.index, data['EMA100'], label='100-day EMA', color='red')
    ax_price.plot(data.index, data['EMA200'], label='200-day EMA', color='blue')

    # Plot Bollinger Bands
    ax_price.plot(data.index, data['bb_middle_band'], label='Middle Band (20-day SMA)', color='black')
    ax_price.plot(data.index, data['bb_upper_band'], label='Upper Band (2 std dev)', color='gray', linestyle='--')
    ax_price.plot(data.index, data['bb_lower_band'], label='Lower Band (2 std dev)', color='gray', linestyle='--')

    ax_price.legend()

    # Plot Volume
    ax_volume.bar(data.index, data['Volume'], color='gray')
    ax_volume.set_ylabel('Volume')

    # Plot RSI
    ax_rsi.plot(data.index, data['RSI'], label='14-day RSI', color='green')
    ...
    ax_rsi.text(data.index[-1], current_rsi, f'{current_rsi:.2f}', color='purple', fontsize=10, ha='left', va='center')
...
def load_tickers():
    ...
    if csv_file:
        ...
        tickers = [f"{ticker.strip()}.NS" for ticker in data[0].tolist()]
...
action_menu = ttk.Combobox(listbox_frame, textvariable=action_var, values=[
    "Squeeze","Momemtum","Momemtum631", "20 Day High Breaking", "20 Day Low Breaking", "Higher High",
...
def load_csv_list():
    ...
    if file_path:
        ...
        return data.iloc[:, 0].tolist(), os.path.basename(file_path)   # Get the first column as a list
    return []
...
def plot_data(ticker, period, interval):
    ...
    data['EMA20'] = ta.trend.ema_indicator(data['Close'].values.flatten() , window=20)
    data['EMA50'] = ta.trend.ema_indicator(data['Close'].values.flatten() , window=50)
    data['EMA100'] = ta.trend.ema_indicator(data['Close'].values.flatten() , window=100)
    data['EMA200'] = ta.trend.ema_indicator(data['Close'].values.flatten() , window=200)
    data['RSI'] = ta.momentum.rsi(data['Close'].values.flatten() , window=14)
    ...
    axs[0].set_title(f'{ticker} - {period} - {interval}')
    axs[0].legend()
    ...
    try:
        ...
        for file_name in os.listdir(folder_path):
            if file_name.endswith(".csv") and file_name != "Cleaned_Files":
                ...
                data_filtered.to_csv(cleaned_file_path, index=False, header=False)
...
def search_rsi():
    ...
    try:
        ...
        for stock in stocks:
            data =  yf.download(stock, period=period, interval=interval, group_by='ticker', progress=True)
            
         
            if isinstance(data.columns, pd.MultiIndex):
                data = data.xs(stock, axis=1, level=0)
...
def save_list():
    try:
        ...
        # Save to CSV
        data = pd.DataFrame(tickers, columns=["Ticker"])
...
def display_old_tickers():
    ...
    try:
        old_data = pd.read_csv(old_file_path, header=None, names=['Ticker'])
...
def display_new_tickers():
    ...
    try:
        new_data = pd.read_csv(new_file_path, header=None, names=['Ticker'])
...
def compare_lists():
    ...
    try:
        ...
        old_data['Rank'] = old_data.index + 1
        new_data['Rank'] = new_data.index + 1

        # Convert to dictionaries with ticker as key and rank as value
        old_list = old_data.set_index('Ticker')['Rank'].to_dict()
        new_list = new_data.set_index('Ticker')['Rank'].to_dict()
...
def save_results():
    ...
    try:
        ...
        data = pd.DataFrame(cleaned_tickers, columns=["Ticker"])
        data.to_csv(save_path, index=False)
...
def save_csv():
    ...
    try:
        ...
        data = pd.DataFrame(results, columns=["Ticker"])
        data.to_csv(save_path, index=False)
...
def analyze_all_files():
    ...
    base_data = yf.download(base_ticker, period=period, interval=interval, group_by='ticker', progress=True)
    if base_data.empty:
           return  # Skip processing for this ticker and move to the next one
...
def plot_chart2(tick):
    ...
    data = yf.download(selected_ticker, period=period_var11.get(), interval=interval_var11.get(),group_by='ticker', progress=True)
    ...
    if isinstance(data.columns, pd.MultiIndex):
        data = data.xs(selected_ticker, axis=1, level=0)
        # Calculate EMAs
    data['EMA20'] = data['Close'].ewm(span=20, adjust=False).mean()
    data['EMA50'] = data['Close'].ewm(span=50, adjust=False).mean()
    data['EMA100'] = data['Close'].ewm(span=100, adjust=False).mean()
    ...
    # Plot EMAs
    ax_price.plot(data.index, data['EMA20'], label=f"20-day EMA {data['EMA20'].iloc[-1]:.2f}", color='orange')
    ax_price.plot(data.index, data['EMA50'], label=f"'50-day EMA {data['EMA50'].iloc[-1]:.2f}", color='purple')
    ax_price.plot(data.index, data['EMA100'], label=f"100-day EMA {data['EMA100'].iloc[-1]:.2f}", color='red')
    ax_price.plot(data.index, data['EMA200'], label=f"200-day EMA {data['EMA200'].iloc[-1]:.2f}", color='blue')

    # Plot Bollinger Bands
    ax_price.plot(data.index, data['bb_middle_band'], label='Middle Band (20-day SMA)', color='black')
    ax_price.plot(data.index, data['bb_upper_band'], label=f"Upper Band- {data['bb_upper_band'].iloc[-1]:.2f}", color='gray', linestyle='--')
    ax_price.plot(data.index, data['bb_lower_band'], label=f"Lower Band {data['bb_lower_band'].iloc[-1]:.2f}", color='gray', linestyle='--')
    ...
    previous_close = data['Close'].iloc[-2]  # Previous candle close
    current_close = data['Close'].iloc[-1]   # Today's closing price
