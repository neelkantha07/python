...
def active_trades():
    ...
    for trade in trades:
        if trade['Status'] == "Active":
            ...
            fast_stoch=fast_stochastic(data,60,10).iloc[-1]
      
            exit_reason = None
            #minstop=min(trade['Stop Loss'], atr)
            #trade['Stop Loss']=minstop
            ...
            if trade['Status'] != "Active":
                ...
                trade['P&L'] = round((-trade['Exit Price'] + trade['Entry']) * trade['Qty'] , 2)
...
def check_trade_conditions(ticker):
    ...
    try:
        ...
        shortfast=fast_stochastic(data,40, 4).iloc[-1]
        fast_stoch=fast_stochastic(data,60,10).iloc[-1]

        trade_in = False  # Initialize before the loop
        for trade in trades:
            if trade['Status'] == "Active" and ticker == trade['Ticker']:
                trade_in = True
                break  
        # Entry Condition
        if fast_stoch < 80 and fast_stoch > 77 and shortfast > 55 and fast_stoch2 > fast_stoch  and price < ema5 and ema5 < ema15 and (not trade_in)and price < 10000:
            entry_price = price
            ...
            trade = {
                ...
                "P&L": 0,
                "Status": "Active",
                "Qty":  1000 if entry_price < 250 else 100 if entry_price < 4000 else 10,
                "Chrg": -50 if entry_price < 100 else -100 if entry_price < 4000 else -190,
