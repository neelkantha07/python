...
tab_market_view=ttk.Frame(notebook)
...
notebook.add(tab_market_view, text=" Market State ")
...
file_menu.add_command(label="Daily Report", command=daily_report)
file_menu.add_command(label="Weekly Report", command=weekly_report)
    ...
    for csv_file in csv_files:
        ...
        for ticker in tickers:
            try:
              ...
              #RS Calculation
                data = pd.concat([base_data['Close'], data['Close']], axis=1)
                data.columns = ['Base', 'Comparative']
                data = data.dropna()
               ...
               # Calculate Relative Strength
                ...
                if (current_rs > 0 and data['RS'].iloc[-1] ):
                    ...
                    listbox_data[category].append((ticker, current_rs))
        ...
        sq_percentage=(sq_count/total_tickers*100) if total_tickers > 0 else 0
        rs_percentage=(rs_count/total_tickers *100 ) if total_tickers > 0 else 0
        summary = (
            f"File: {csv_file}\n"
            ...
            f"Relative Strength: {rs_count} ({rs_percentage:.2f}%)\n"
            f"Squeeze: {sq_count} ({sq_percentage:.2f}%)\n"
            f"{'-'*50}\n"
        )
...
def display_comparison(file1, file2):
    ...
    for file_name in all_files:
        ...
        for category in ["Overbought", "Oversold", "Gaining Strength", "Losing Strength","RS","Sq"]:
...
def save_rs():
    listbox11 = listbox_widgets["RS"]
    matching_stocks = listbox11.get(0, tk.END)
    if not matching_stocks:
        messagebox.showinfo("Save Error", "No tickers to save.")
        return
    ...
    # Extract ticker names from the listbox
        for item in listbox11.get(0, listbox11.size() - 1):
            ticker = item.split(':')[0].strip()  # Get the ticker name before the ':'
            ticker_base = ticker.split('.NS')[0]  # Remove the '.NS' part
            tickers.append([ticker_base])  # Append as a list to match CSV writer format
    
        # Save ticker names to a CSV file
        try:
        # Save ticker names to a CSV file
            with open(save_file, 'w', newline='') as file:
                writer = csv.writer(file)
                writer.writerows(tickers)  # Save without header
    
            # Show confirmation message
            messagebox.showinfo("Save Successful", f"Tickers have been saved to {save_file}")
    
        except Exception as e:
        # Show error message if saving fails
            messagebox.showerror("Save Failed", f"Failed to save the file. Error: {e}")#
def save_sq():
    listbox11 = listbox_widgets["Sq"]
    matching_stocks = listbox11.get(0, tk.END)
    if not matching_stocks:
        messagebox.showinfo("Save Error", "No tickers to save.")
        return
...
def save_gaining():
    listbox11 = listbox_widgets["Gaining Strength"]
    matching_stocks = listbox11.get(0, tk.END)
    if not matching_stocks:
        messagebox.showinfo("Save Error", "No tickers to save.")
        return
    ...
    # Extract ticker names from the listbox
        ...
        # Show error message if saving fails
            messagebox.showerror("Save Failed", f"Failed to save the file. Error: {e}")## GUI Setup
...
compare_frame.grid(row=5, column=3, columnspan=3, pady=1)
...
file_button2 = tk.Button(tab_market_view, text="Browse single file", command=load_folder)
file_button2.grid(row=0, column=3, pady=10)
...
save_button_gaining = tk.Button(tab_market_view, text="Save Gaining", command=save_gaining)
save_button_gaining.grid(row=7,column=1,pady=0)

save_button_rs = tk.Button(tab_market_view, text="Save RS", command=save_rs)
save_button_rs.grid(row=7,column=2,pady=0)

save_button_sq = tk.Button(tab_market_view, text="Save Sq", command=save_sq)
save_button_sq.grid(row=7,column=3,pady=0)



categories = ["Overbought", "Oversold", "Gaining Strength", "Losing Strength","RS","Sq"]
listbox_widgets = {}
