...
def load_tickers_from_csv():
    ...
    if os.path.exists(csv_file_path):
        try:
            tickers_data = pd.read_csv(csv_file_path, header=None)
            monitored_tickers = tickers_data[0].tolist()  # Extract the tickers column
            update_listboxes()
...
def stochastic(data, period=60, k_smooth=10):
    data['L'] = data['Low'].rolling(period).min()
    data['H'] = data['High'].rolling(period).max()
    data['%K'] = 100 * ((data['Close'] - data['L']) / (data['H'] - data['L']))
    data['%K'] = data['%K'].rolling(k_smooth).mean()
    return data
...
def active_trades():
    ...
    for trade in trades:
        if trade['Status'] == "Active":
            ticker = trade['Ticker']
            data = yf.download(ticker, period="5d", interval="1m", group_by='ticker', progress=True)
            ...
            data['EMA5'] = ema(data, 5)
            data['EMA15'] = ema(data, 15)
            ...
            atr=calculate_supertrend((data))
            #trade['Stop Loss']=min(trade['Stop Loss'], atr)
            trade['CP']=price
            trade['P&L'] = round((trade['CP'] - trade['Entry']) * trade['Qty'] + trade['Chrg'], 2)
...
def check_trade_conditions(ticker):
    ...
    try:
        ...
        ema15 = last_row['EMA15']
        ndlast = data.iloc[-2]
        fast_stoch7 = fast_stochastic(data, 60, 3)
        fast_stoch2 = ndlast['%K']
        fast40=fast_stochastic(data, 40, 4).iloc[-1]
        ...
        for trade in trades:
            if trade['Status'] == "Active" and ticker == trade['Ticker']:
                ...
                break  
        # Entry Condition
        if fast_stoch >=80 and fast_stoch7.iloc[-1]>fast_stoch7.iloc[-2] and fast_stoch7.iloc[-2]>fast_stoch7.iloc[-3]  and fast_stoch7.iloc[-3]>fast_stoch7.iloc[-4] and fast_stoch7.iloc[-4]>fast_stoch7.iloc[-5] and fast40 > 70 and price > ema5 and ema5 > ema15 and (not trade_in)  :
            entry_price = price
            stop_loss = entry_price * (1 - 0.0026)  # -0.26%
            ...
            trade = {
                ...
                "CP":0,
                "Status": "Active",
                "Qty":  500 if entry_price < 250 else 100 if entry_price < 4000 else 10,
                "Chrg": -50 if entry_price < 100 else -70 if entry_price < 4000 else -100,
                }
            trades.append(trade)
...
def update_listboxes():
    ...
    for trade in trades:
        if(trade['Status'] == "Active"):
            trade_listbox.insert(tk.END, f"{trade['Ticker']} | Entry: {trade['Entry']} | Qty: {trade['Qty']} | LP: {trade['CP']}")
